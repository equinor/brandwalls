...
const fetchData = async (url: string) => {
  ...
  if (!response.ok) {
    const error = new Error(
      "An error occurred while fetching StockValues data.",
    );
    throw error;
  }
...
const StockValues = () => {
  const { data, error, isLoading } = useSWR(ENDPOINT, fetchData, {
    ...
    return (
      <div className="flex h-full w-full flex-col items-start justify-start p-4">
        <div className="flex items-baseline gap-1 *:leading-none">
          <div className="font-normal text-6xl text-norwegian-woods-100">
            {isLoading ? (
          ...
          <div className="font-normal text-2xl">
            {isLoading || error || !data ? "" : currency}
          </div>
        </div>
        <div className="font-light text-xl">
          {isLoading || error || !data ? "" : `${change}%`}
        </div>
        <h2 className="mt-4 font-normal text-xl">
          {isLoading || error || !data ? "Oslo Stock Exchange (OSE)" : title}
        </h2>
        <div className="font-normal text-lg">
          {isLoading || error || !data
            ? ""
            : format(new Date(date), "d LLLL yyyy hh:mm (z)")}
        </div>
      </div>
    );
  };
  const getNYSETemplate = (
    price: string,
    currency: string,
    change: string,
    title: string,
